...
const VideoChatScreen = () => {
	...
	const { alert, loading, postVideoToServer, progress } = useRecordUpload(
		userProfile?.token,
		classId
	);
			...
			.then((stream) => {
				...
				presentationPeer.current.on("call", (call) => {
					...
					call.on("stream", (userVideoStream) => {
						console.log(
							"remote presentation viewer sent sream: ",
							userVideoStream
						);
					});
	...
	const [remoteUserPresentingProccessing, setRemoteUserPresentingProccessing] =
		useState(false);
	...
	const startWebCam = async () => {
		...
		socket.on("incoming-raising-hand", (userData) => {
			...
			sessionStorage.setItem(
				HandKey,
				JSON.stringify([...handRaiseList, userData])
			);
		...
		socket.on("incoming-unraising-hand", (userData) => {
			...
			let newList = currentList.filter(
				(item) => item.name !== `${userData.firstName} ${userData.lastName}`
			);
	...
	const handleSubmit = (e) => {
		...
		toast.info(
			`You: ${userProfile.firstName} ${userProfile.lastName} - ${value}`
		);
	...
	function unRaiseHand() {
		...
		let newList = currentList.filter(
			(item) => item.name !== `${userProfile.firstName} ${userProfile.lastName}`
		);
	...
	return (
		<Wrapper>
			...
			<Content>
				<VideoWrapper isPresenting={isPresenting}>
					...
					<UserPresentation isPresenting={isPresenting}>
						<video
							className="client-presentation-stream"
							src=""
							muted={true}
						></video>
					</UserPresentation>
					<StreamWrapper isPresenting={isPresenting} className="video-section">
						<UserCallBlock
							showText={localStream.current?.getVideoTracks()[0].enabled}
						>
							<video
								className="client-local-stream"
								src=""
								muted={true}
							></video>
							<p>
					...
					<ControlWrapper>
						...
						<div className="controls">
							...
							<ControlItem
								...
								onClick={() => setOpenToolBox(true)}
							>
								<HiDotsVertical size="1.5rem" />
							</ControlItem>
							<ControlItem
								isOn={true}
								onClick={handRaised ? unRaiseHand : raiseHand}
							>
								<HiOutlineHand size="1.5rem" />
							</ControlItem>
							<ControlItem
								onClick={togggleAudio}
								isOn={callSettingsState.audio}
							>
								{callSettingsState.audio ? (
									<BsMic size="1.5rem" />
								) : (
									<BsMicMute size="1.5rem" />
								)}
							</ControlItem>
							<ControlItem
							...
							<ControlItem
								onClick={togggleVideo}
								isOn={callSettingsState.video}
							>
								{callSettingsState.video ? (
									<BsCameraVideo size="1.5rem" />
								) : (
									<BsCameraVideoOff size="1.5rem" />
								)}
							</ControlItem>
			...
			>
				<Box sx={style}>
					<header style={{ marginBottom: "0" }}>Live-Chat</header>
					<div className="boxtop">
						{chatMessages?.length > 0 &&
							chatMessages?.map((x) => (
								<div className={`message ${x.mine && "mine"}`}>
									<p style={{ color: "#0C2191", marginBottom: ".5rem" }}>
										{x.name}
									</p>
									<span> {x.value}</span>
								</div>
...
function SuperWrapper() {
	...
	const {
		...
		teacherFunctions: { fetchBootcamps: fetchTeacherBootcamps },
	} = useAuth();
	...
	const fetchStudentApplications = useQuery(
		["fetchStudentApplications", userProfile.token],
		() => fetchBootcamps(userProfile.token),
		{
			enabled: userProfile.userType === "student",
			onSuccess: (res) => {
				if (res.statusCode === 1) {
					const findMyClasss = res.data.find(
						(item) => item.bootcampId === classId
					);
	...
	const fetchTeacherApplications = useQuery(
		["fetchTeacherApplications", userProfile.token],
		() => fetchTeacherBootcamps(userProfile.token),
		{
