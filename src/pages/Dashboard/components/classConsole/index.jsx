...
export const Console = ({ children }) => {
	...
	const studentAssessMent =
		pathname.includes("/student/console/myclasses") ||
		pathname.includes("/student/console/assessments") ||
		pathname.includes("/student/console/messages") ||
		pathname.includes("/student/console/answers");
	...
	return (
		<div className={style.console}>
			<Sidebar Toggle={Toggle} toggleModule={toggleModule} />
			<ModalContent
				show={show}
				setShow={setShow}
...
function Sidebar({ Toggle, side, toggleModule }) {
	...
	const domainUpdate = useMutation(
		([token, data, id]) => updateDomain(token, data, id),
		{
			onSuccess: (res) => {
				if (res.statusCode !== 1) {
	...
	function onDragEnd(result) {
		...
		newData.forEach((item, i) => organisedData.push({ ...item, order: i + 1 }));
					...
					{!studentpath ? (
						<>
							...
							<Link className="d-inline-flex" to={goBack()}>
								<button className={style.back_button} style={{ width: "100%" }}>
									Back to Dashboard
								</button>
							</Link>
						</>
                        ...
                        <BsThreeDotsVertical />
								...
								<div
									className={`${style.create_content_button} ${style.orange}`}
								>
...
export function Accord({
	...
	const userdata = getItem(KEY);
	const {
		consoleFunctions: { fetchContents, addFile, updateContent },
	} = useAuth();
	const [domainContent, setDomainContent] = useState([]);
	const queryClient = useQueryClient();

	const getDomainContent = useQuery(
		["getDomainContent", classId],
		() => fetchContents(userdata.token, classId),
		{
			onSuccess: (res) => {
				if (res.success) {
				...
				(creator ? (
					mutation.isLoading ? (
						...
						<ul className={style.content_list}>
							{getDomainContent?.data?.data
								?.filter((item) => item.domain === _id)
								.filter((item) => item?.type === "FILE_VIDEO")
								.map(
									({ icon: Icon, title, link, _id, type, domain, classId }) => (
										<li
											key={_id}
...
function AccordMenu({
	...
	return (
		<div style={{ marginLeft: "auto" }}>
			...
			>
				<MenuItem component="div" onClick={editContent}>
					Edit content
				</MenuItem>
				{locked ? (
					<MenuItem onClick={() => handleLockToggle("unlock")}>
						Unlock content
					</MenuItem>
				) : (
					<MenuItem onClick={() => handleLockToggle("lock")}>
						Lock content
					</MenuItem>
				)}
...
export function ModalContent({ show, handleClose, toggleModule, editData }) {
	...
	let ref = useRef();
	const {
		consoleFunctions: { addContent, updateContent },
	} = useAuth();
	...
	const queryClient = useQueryClient();
	const fetchDomains = useQuery(["fetch domains", classId], () =>
		fetchDomains(userdata.token, classId)
	);

	const addContentMutation = useMutation(
		([token, state]) => addContent(token, state),
		{
			onSuccess: (res) => {
				queryClient.invalidateQueries("fetch domains");
				if (res.statusCode !== 1) {
	...
	function editContent() {
		editContentMutation.mutate([
			userdata.token,
			{ ...formstate, classId, domainId: formstate.domain },
			editData._id,
		]);
	}
		...
		<div>
			<Modal show={show} onHide={handleClose}>
				<Modal.Header closeButton className={style.modal__header}>
					<Modal.Title className={style.modal__title}>
					...
					<div className={style.content__form}>
						...
						{!editData?._id && (
							<FormControl>
								...
								<FormHelperText>
									A Domain is a container for similar content. e.g
									"Introduction", "Day 1" or "Domain 1"
								</FormHelperText>
							</FormControl>
						)}
								...
								<div>
									...
									<div className={style.switchBorder}>
										...
										<p className={style.formtext}>
											...
											{formstate.isLocked ? "locked" : "unlocked"}
										</p>
									</div>
						...
						{editData?._id ? (
							<button
								className={style.contentform__btn}
								onClick={editContent}
								disabled={addContentMutation.isLoading}
							>
								{editContentMutation.isLoading ? (
...
export function ModuleModal({ moduleOpen, moduleClose, editModule }) {
	const {
		consoleFunctions: { addDomain, updateDomain },
	} = useAuth();
	const { getItem } = useLocalStorage();
	const [formstate, setFormstate] = useState({});
	const queryClient = useQueryClient();
...
export function PopModalContent({
	open,
	closeSmall,
	openUpload,
	setScreenOpen,
...
export function MainContainer() {
	...
	const getDomainContent = useQuery(["getDomainContent", classId], () =>
		fetchContents(userdata.token, classId)
	);
	...
	useEffect(() => {
		if (getDomainContent?.data?.data?.length > 0) {
			if (contentid) {
				let content = getDomainContent.data.data.find(
